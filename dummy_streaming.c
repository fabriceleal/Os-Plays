/**
 * @file dummy_streaming.c
 * @author Fabrice Leal <fabrice [dot]leal[dot]ch[at]gmail[dot]com>
 *
 * @section LICENSE
 *
 * < ... goes here ... >
 *
 * @section DESCRIPTION
 * 
 * To be used in filesystem generators, not by the os. This is "standard" C.
 *
 */

#include "dummy_streaming.h"


/*
	Inits the buffer structure.
*/
int init(buffer * obj)
{
	if (obj == NULL)
	{
		return BUFFER_NULL_PTR;
	}

	obj->len = 0;
	obj->buffer_size = 0;
	obj->buffer = NULL;

	return BUFFER_SUCCESS;
}

/*
	Writes a array of chars into buffer. Allocates dynamically if necessary.
*/
int write(buffer * dest, const char * write, const unsigned int len)
{
	if(obj == NULL)
	{
		return BUFFER_NULL_PTR;
	}

	if( len == 0 || write == NULL )
	{
		return BUFFER_INVALID_PARAMETER;
	}

	int current_end = buffer->len;
	
	if(buffer->buffer == NULL || len + buffer->len > buffer->buffer_size )
	{
		// New alloc is needed
		int new_size = (len + buffer->len);
		char * new_ptr = malloc( sizeof(char) * new_size );

		if(new_ptr == NULL)
		{
			return BUFFER_NO_MEM;
		}
		
		// Copy data to new ptr
		memcpy( new_ptr, buffer->buffer, buffer->len );

		// Free old ptr
		free( buffer->buffer );
		
		// Update structure
		buffer->buffer = new_ptr;
		// In this implementation, buffer_size == data_len
		buffer->len = buffer->buffer_size = new_size;
	}

	// Append new data
	memcpy( buffer->buffer + current_end, write, len);

	return BUFFER_SUCCESS;
}

/*
	Releases de internal data inside the supplied buffer. 
	The pointer of the buffer itself needs to be free'd by the caller.
*/
int destroy(buffer * obj)
{
	if(obj == NULL)
	{
		return BUFFER_NULL_PTR;
	}

	if(obj->buffer != NULL)
	{
		free( obj->buffer );
	}

	obj->buffer = NULL;
	obj->buffer_size = 0;
	obj->len = 0;

	return BUFFER_SUCCESS;
}
